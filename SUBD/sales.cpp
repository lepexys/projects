...
sales::sales(QWidget *parent,QSqlDatabase *db) : QWidget(parent)
  ,ui(new Ui::sales)
{
    ui->setupUi(this);
    ...
    mdl->setHeaderData(2,Qt::Horizontal,"№ Клиента");
...
void sales::on_pushButton_Add_clicked()
{
            ...
            && !ui->lineEdit_10->text().isEmpty() && !ui->lineEdit_5->text().isEmpty())
    {
        ...
        query.prepare(QString("INSERT INTO sales (product_id, client_id, date, price_per_unit, amount, VAT_sum, storage, contract, unit, order_id) "
                              "VALUES ( :product_id, :client_id, :date, :price_per_unit, :amount, :vat_sum, :storage, :contract, :unit, :order_id)"));
        query.bindValue(":product_id",QString(ui->lineEdit_4->text()));
        query.bindValue(":client_id",QString(ui->lineEdit_6->text()));
        ...
        query.bindValue(":amount",QString::number(ui->lineEdit_2->text().toFloat(),'f',2));
        ...
        query.bindValue(":storage",QString(ui->lineEdit_9->text()));
        query.bindValue(":contract",QString(ui->lineEdit_10->text()));
        QSqlQuery query1(mdl->database());
        query1.exec("SELECT units FROM goods WHERE id = "+QString(ui->lineEdit_4->text()));
        if(query1.next())
        {
        ...
        else
        {
            ...
            if(query4.next())
            {
                query5.exec("UPDATE storage SET amount = amount - "+QString::number(ui->lineEdit_2->text().toFloat(),'f',2)+" WHERE product_id = "+ui->lineEdit_4->text()+" AND code = "+QString(ui->lineEdit_9->text()));
            }
            else
            {
...
void sales::on_pushButton_Ret_clicked()
{
    mdl->revertAll();
    if(!mdl->select())
        QMessageBox::critical(NULL,QObject::tr("Ошибка"),mdl->lastError().text());
...
{
    clients* form = new clients(nullptr,new QSqlDatabase(mdl->database()),true);
    connect(form,SIGNAL(ret_id(int)),this,SLOT(id_2(int)));
    form->show();
}

void sales::on_pushButton_1_clicked()
{
