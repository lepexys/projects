...
clients::clients(QWidget *parent,QSqlDatabase *db, bool d) : QWidget(parent)
  ,ui(new Ui::clients)
{
    ui->setupUi(this);
    mdl = new QSqlTableModel(this,*db);
    mdl->setTable("clients");
    mdl->select();
...
{
    QString q = QString("INSERT INTO clients (firm_name, dirrector, accountant_general, adress, telephone, requisites, tin) "
...
void clients::on_pushButton_Del_clicked()
{
    if(mdl->rowCount()!=0)
    {
...
void clients::on_pushButton_Cho_clicked()
{
    if(mdl->rowCount()!=0)
    {
        int selRow = ui->tableView->currentIndex().row();
