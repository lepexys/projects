...
class Product : public Coding
{
	...
	CalcuProd* kalkul; //Массив калькуляций
...
public:
	void InitProd()
	{
		...
		for (int i = 0; i < rang; i++)
		{
			...
			f >> buff >> buff;
			t[i].colcal = 0;
			t[i].kalkul = (CalcuProd*)malloc(sizeof(class CalcuProd));
			while (!strcmp(buff, "*") == 0)
			{
				t[i].kalkul = (CalcuProd*)realloc(t[i].kalkul, 1 * sizeof(class CalcuProd));
				f >> weight;
				CalcuProd  obj;
				obj.AddCalcu(t[i].kalkul[t[i].colcal], t[i].getcode(), buff, weight);
				f >> buff;
				t[i].colcal++;
			}
		}
...
public:
	...
	void AddCalculation(int u)
	{
		...
		while (k == 1)
		{
			t[u].kalkul = (CalcuProd*)realloc(t[u].kalkul, 1 * sizeof(class CalcuProd));
			cout << "Введите тип сырья: ";
			cin >> sor;
			cout << "Введите колличество сырья: ";
			cin >> weight;
			CalcuProd  obj;
			obj.AddCalcu(t[u].kalkul[colcal], t[u].getcode(), sor, weight);
			t[u].colcal++;
...
public:
	...
	int add()
	{
		...
		cin >> bname;
		t[rang].name = (char*)calloc(strlen(bcode) + 1, sizeof(char));
